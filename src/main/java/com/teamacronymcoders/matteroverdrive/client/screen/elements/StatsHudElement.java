...
import com.teamacronymcoders.matteroverdrive.api.android.gui.HudElement;
import com.teamacronymcoders.matteroverdrive.api.android.gui.HudPosition;
...
public class StatsHudElement extends HudElement {
    ...
    public static int renderIconWithInfo(PoseStack stack, ResourceLocation holoIcon, String info, Color color, int x, int y, int iconOffsetX, int iconOffsetY, boolean leftSided, int iconWidth, int iconHeight, float backgroundAlpha) {
        ...
        //MORenderUtil.fillRect(stack, x, y - 1, x + infoWidth + 2 + iconWidth + 2, y + iconHeight, backgroundColor.getRGB(), backgroundColor.getAlpha());
    ...
    public void drawElement(PoseStack stack, IAndroid androidPlayer, Window resolution, float ticks) {
        ...
        if (this.getPosition().getY() == 0 || this.getPosition().getY() == 1){
            ...
            // Draw Shaded Background
            //Color backgroundColor = new Color(0, 0, 0, backgroundAlpha);
            //MORenderUtil.fillRect(stack, x + 2, y + 1, getWidth(resolution, androidPlayer) + 10, getHeight(resolution, androidPlayer) - 11, backgroundColor.getRGB(), backgroundColor.getAlpha());
