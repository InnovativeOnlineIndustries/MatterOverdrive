 ...
 * matteroverdrive.tile.TileEntityWeaponStation; import
 ...
 *//**
	 ...
	 *//*
		 ...
		 * 
		 * if (te instanceof TileEntityWeaponStation) { TileEntityWeaponStation
		 * weaponStation = (TileEntityWeaponStation) te;
		 * 
		 * if (!weaponStation.getStackInSlot(weaponStation.INPUT_SLOT).isEmpty()) {
		 * String name =
		 * weaponStation.getStackInSlot(weaponStation.INPUT_SLOT).getDisplayName();
		 * currenttip.add(TextFormatting.YELLOW + "Current Weapon: " +
		 * TextFormatting.WHITE + name); }
		 * 
		 * } else { throw new
		 * RuntimeException("Weapon Station WAILA provider is being used for something that is not a Weapon Station: "
		 * + te.getClass()); }
		 * 
		 * return currenttip; }
		 * 
